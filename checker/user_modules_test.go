...
fn main() Int {
	...
	result := parse.Parse([]byte(mainContent), "main.ard")
	if len(result.Errors) > 0 {
		t.Fatal(result.Errors[0].Message)
	}
	astTree := result.Program
...
fn func1() Int {
	...
	result1 := parse.Parse([]byte(content1), "main1.ard")
...
fn func2() Int {
	...
	result2 := parse.Parse([]byte(content2), "main2.ard")
...
func TestUserModuleErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := parse.Parse([]byte(tt.input), "main.ard")
			if len(result.Errors) > 0 {
				t.Fatal(result.Errors[0].Message)
			}
			astTree := result.Program
...
`
	...
	result := parse.Parse([]byte(moduleContent), "main.ard")
	if len(result.Errors) > 0 {
		t.Fatal(result.Errors[0].Message)
	}
	astTree := result.Program
