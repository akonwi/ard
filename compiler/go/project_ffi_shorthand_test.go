...
func TestRunProgramCanonicalizesProjectFFIImportShorthand(t *testing.T) {
	...
	if err := os.WriteFile(mainPath, []byte("use go:app/ffi\n\nfn main() {\n  if not ffi::Answer() == 42 { panic(\"wrong answer\") }\n}\n"), 0o644); err != nil {
		t.Fatal(err)
	}
