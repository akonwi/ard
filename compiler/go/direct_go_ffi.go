...
func (l *lowerer) coerceDirectGoArg(ardType air.TypeID, arg ast.Expr, goType checker.GoValueType, binding directGoExternBinding) (ast.Expr, error) {
	...
	// A callback whose Ard parameter/result types lower to exactly the target
	// Go func signature passes straight through with no adapter shim. The
	// checker rejects any closure whose lowered signature would differ, so this
	// is always safe.
