...
func main() {
	...
	vmAdapters, err := renderVMNextAdapters(c)
	if err != nil {
		panic(err)
	}
	if err := os.WriteFile(filepath.Join("..", "..", "vm_next", "ffi_stdlib_adapters.gen.go"), vmAdapters, 0o644); err != nil {
		panic(err)
	}
}
...
func renderVMNextAdapters(c contract) ([]byte, error) {
	file := &ast.File{Name: ast.NewIdent("vm_next")}
	file.Decls = append(file.Decls, &ast.GenDecl{
		Tok: token.IMPORT,
		Specs: []ast.Spec{
			&ast.ImportSpec{Path: stringLit("fmt")},
			&ast.ImportSpec{Path: stringLit("github.com/akonwi/ard/air")},
			&ast.ImportSpec{Name: ast.NewIdent("stdlibffi"), Path: stringLit("github.com/akonwi/ard/std_lib/ffi")},
		},
	})

	cases := make([]ast.Stmt, 0, len(c.Functions))
	for _, fn := range c.Functions {
		clause, err := vmNextAdapterCase(c, fn)
		if err != nil {
			return nil, err
		}
	...
	file.Decls = append(file.Decls, &ast.FuncDecl{
		Name: ast.NewIdent("generatedStdlibHostExternAdapter"),
		Type: &ast.FuncType{
			Params: fields(
...
func vmNextAdapterCase(c contract, fn hostFunction) (ast.Stmt, error) {
	sig, err := parseGoExpr(vmNextFuncType(c, fn))
	if err != nil {
	...
	body := []ast.Stmt{
		...
		returnStmt(vmNextAdapterFuncLit(c, fn), ast.NewIdent("true")),
	}
...
func vmNextAdapterFuncLit(c contract, fn hostFunction) ast.Expr {
	stmts := make([]ast.Stmt, 0, len(fn.Params)*2+3)
	for i, param := range fn.Params {
		argName := fmt.Sprintf("arg%d", i)
		argType := mustParseGoExpr(qualifyVMNextType(param.Type, c))
		stmts = append(stmts,
			&ast.AssignStmt{
...
func vmNextReturnCall(returnType, outName string) ast.Expr {
	...
	case strings.HasPrefix(returnType, "Result["):
		helper = "stdlibHostReturnResult"
	case returnType == "error":
		helper = "stdlibHostReturnError"
	default:
		helper = "stdlibHostReturnValue"
	}
	return callExpr(ast.NewIdent(helper), ast.NewIdent("vm"), selector("extern", "Signature", "Return"), ast.NewIdent(outName))
}

func vmNextFuncType(c contract, fn hostFunction) string {
	params := make([]string, 0, len(fn.Params))
	for _, param := range fn.Params {
		params = append(params, qualifyVMNextType(param.Type, c))
	}
	returns := make([]string, 0, len(fn.Returns))
	for _, ret := range fn.Returns {
		returns = append(returns, qualifyVMNextType(ret, c))
	}
...
func qualifyVMNextType(goType string, c contract) string {
