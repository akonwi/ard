...
func main() {
	...
	flag.StringVar(&opts.vmOut, "vm-out", "vm_adapters.gen.go", "generated vm adapter output")
	flag.StringVar(&opts.vmAdapterFunc, "vm-adapter-func", "VMAdapter", "generated vm adapter lookup function")
	flag.Parse()
	...
	vmAdapters, err := renderVMAdapters(c, opts)
	if err != nil {
		panic(err)
	}
	if err := os.WriteFile(opts.vmOut, vmAdapters, 0o644); err != nil {
		panic(err)
	}
}
...
func renderVMAdapters(c contract, opts generateOptions) ([]byte, error) {
	...
	file.Decls = append(file.Decls, &ast.GenDecl{
		...
		Specs: []ast.Spec{
			...
			&ast.ImportSpec{Name: ast.NewIdent("vmffi"), Path: stringLit("github.com/akonwi/ard/vm/ffi")},
		},
	})
	file.Decls = append(file.Decls, vmAdapterSupportDecls()...)

	cases := make([]ast.Stmt, 0, len(c.Functions))
	for _, fn := range c.Functions {
		clause, err := vmAdapterCase(c, opts, fn)
		if err != nil {
			return nil, err
		}
	...
	file.Decls = append(file.Decls, &ast.FuncDecl{
		Name: ast.NewIdent(opts.vmAdapterFunc),
		Type: &ast.FuncType{
			Params: fields(
			...
			Results: fields(
				field("", selector("vmffi", "ExternAdapter")),
...
func vmAdapterSupportDecls() []ast.Decl {
	return []ast.Decl{
		&ast.FuncDecl{
			Name: ast.NewIdent("init"),
			Type: &ast.FuncType{Params: fields()},
			Body: &ast.BlockStmt{List: []ast.Stmt{
				&ast.ExprStmt{X: callExpr(selector("vmffi", "RegisterAdapterLookup"), ast.NewIdent("VMAdapter"))},
			}},
		},
		&ast.FuncDecl{
...
func vmAdapterCase(c contract, opts generateOptions, fn hostFunction) (ast.Stmt, error) {
	sig, err := parseGoExpr(vmFuncType(fn))
	if err != nil {
	...
	body := []ast.Stmt{
		...
		returnStmt(vmAdapterFuncLit(c, opts, fn), ast.NewIdent("true")),
	}
...
func vmAdapterArgStmts(argName string, index int, goType string) []ast.Stmt {
	...
	argType := mustParseGoExpr(qualifyVMType(goType))
...
func vmAdapterFuncLit(c contract, opts generateOptions, fn hostFunction) ast.Expr {
	stmts := make([]ast.Stmt, 0, len(fn.Params)*2+3)
	for i, param := range fn.Params {
		argName := fmt.Sprintf("arg%d", i)
		stmts = append(stmts, vmAdapterArgStmts(argName, i, param.Type)...)
	}
	...
	case 1:
		...
		stmts = append(stmts, returnStmt(vmReturnCall(fn.Returns[0], "out0")))
	case 2:
	...
	return &ast.FuncLit{
		Type: &ast.FuncType{
			Params: fields(
				field("bridge", selector("vmffi", "Bridge")),
...
func vmReturnCall(returnType, outName string) ast.Expr {
...
func vmFuncType(fn hostFunction) string {
	params := make([]string, 0, len(fn.Params))
	for _, param := range fn.Params {
		params = append(params, qualifyVMType(param.Type))
	}
	returns := make([]string, 0, len(fn.Returns))
	for _, ret := range fn.Returns {
		returns = append(returns, qualifyVMType(ret))
	}
...
func qualifyVMType(goType string) string {
	var out strings.Builder
