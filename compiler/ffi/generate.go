...
type generateOptions struct {
	...
	hostPackage       string
	vmNextOut         string
	vmNextPackage     string
	vmNextAdapterFunc string
	hostImportAlias   string
...
func main() {
	...
	flag.StringVar(&opts.vmNextOut, "vm-next-out", filepath.Join("..", "..", "vm_next", "ffi_stdlib_adapters.gen.go"), "generated vm_next adapter output")
	flag.StringVar(&opts.vmNextPackage, "vm-next-package", "vm_next", "package name for generated vm_next adapters")
	flag.StringVar(&opts.vmNextAdapterFunc, "vm-next-adapter-func", "generatedStdlibHostExternAdapter", "generated vm_next adapter lookup function")
	flag.StringVar(&opts.hostImportAlias, "host-import-alias", "stdlibffi", "import alias for the generated host contract package in vm_next adapters")
	flag.StringVar(&opts.hostImportPath, "host-import-path", "github.com/akonwi/ard/std_lib/ffi", "import path for the generated host contract package in vm_next adapters")
	flag.Parse()
...
func renderVMNextAdapters(c contract, opts generateOptions) ([]byte, error) {
	file := &ast.File{Name: ast.NewIdent(opts.vmNextPackage)}
	file.Decls = append(file.Decls, &ast.GenDecl{
		Tok: token.IMPORT,
		Specs: []ast.Spec{
			&ast.ImportSpec{Path: stringLit("fmt")},
			&ast.ImportSpec{Path: stringLit("github.com/akonwi/ard/air")},
			&ast.ImportSpec{Name: ast.NewIdent(opts.hostImportAlias), Path: stringLit(opts.hostImportPath)},
		},
	})
	...
	file.Decls = append(file.Decls, &ast.FuncDecl{
		...
		Body: &ast.BlockStmt{List: []ast.Stmt{
			&ast.ExprStmt{X: callExpr(ast.NewIdent("registerGeneratedHostExternAdapter"), ast.NewIdent(opts.vmNextAdapterFunc))},
		}},
	})
	...
	file.Decls = append(file.Decls, &ast.FuncDecl{
		...
		Type: &ast.FuncType{
			...
			Results: fields(
				field("", ast.NewIdent("hostExternDirect")),
...
func vmNextAdapterCase(c contract, opts generateOptions, fn hostFunction) (ast.Stmt, error) {
	sig, err := parseGoExpr(vmNextFuncType(c, opts, fn))
	if err != nil {
...
func vmNextAdapterFuncLit(c contract, opts generateOptions, fn hostFunction) ast.Expr {
	...
	for i, param := range fn.Params {
		...
		argType := mustParseGoExpr(qualifyVMNextType(param.Type, c, opts.hostImportAlias))
		stmts = append(stmts,
			&ast.AssignStmt{
			...
			&ast.IfStmt{
				...
				Body: &ast.BlockStmt{List: []ast.Stmt{returnStmt(&ast.CompositeLit{Type: ast.NewIdent("Value")}, callExpr(selector("fmt", "Errorf"), stringLit("extern %s arg "+strconv.Itoa(i)+": %w"), ast.NewIdent("binding"), ast.NewIdent("err")))}},
			},
		)
	}
	...
	return &ast.FuncLit{
		Type: &ast.FuncType{
			Params: fields(
				field("vm", &ast.StarExpr{X: ast.NewIdent("VM")}),
				field("extern", selector("air", "Extern")),
				field("binding", ast.NewIdent("string")),
				field("args", &ast.ArrayType{Elt: ast.NewIdent("Value")}),
			),
			Results: fields(
				field("", ast.NewIdent("Value")),
...
func vmNextFuncType(c contract, opts generateOptions, fn hostFunction) string {
	...
	for _, param := range fn.Params {
		params = append(params, qualifyVMNextType(param.Type, c, opts.hostImportAlias))
	}
	returns := make([]string, 0, len(fn.Returns))
	for _, ret := range fn.Returns {
		returns = append(returns, qualifyVMNextType(ret, c, opts.hostImportAlias))
	}
...
func qualifyVMNextType(goType string, c contract, hostImportAlias string) string {
	qualified := vmNextQualifiedTypeNames(c)
	var out strings.Builder
	for i := 0; i < len(goType); {
		...
		if r == '_' || unicode.IsLetter(r) {
			...
			if _, ok := qualified[ident]; ok {
				out.WriteString(hostImportAlias)
				out.WriteString(".")
			}
...
func vmNextQualifiedTypeNames(c contract) map[string]struct{} {
	...
	for _, alias := range c.Aliases {
		out[alias.Name] = struct{}{}
	}
	for _, enum := range c.Enums {
		out[enum.Name] = struct{}{}
	}
	for _, strct := range c.Structs {
		out[strct.Name] = struct{}{}
	}
	return out
}
