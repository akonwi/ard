...
func TestStdlibImportTargetValidation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			source:      "use ard/env\nfn main() Int { 1 }",
			wantErrPart: "Cannot import ard/env when targeting js-browser; allowed targets: vm_next, go, js-server",
		},
		{
		...
		},
		{
			...
			target:      backend.TargetJSBrowser,
			source:      "use ard/fs\nfn main() Int { 1 }",
			wantErrPart: "Cannot import ard/fs when targeting js-browser; allowed targets: vm_next, go, js-server",
		},
		{
		...
		},
		{
			...
			target:      backend.TargetJSBrowser,
			source:      "use ard/sql\nfn main() Int { 1 }",
			wantErrPart: "Cannot import ard/sql when targeting js-browser; allowed targets: vm_next, go",
		},
		{
		...
		},
		{
			...
			target:      backend.TargetJSBrowser,
			source:      "use ard/io\nfn main() Int { 1 }",
			wantErrPart: "Cannot import ard/io when targeting js-browser; allowed targets: vm_next, go, js-server",
		},
		{
