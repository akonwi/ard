...
func (vm *VM) evalResultMethod(scope *scope, self *runtime.Object, call *checker.FunctionCall) *runtime.Object {
	switch call.Name {
	case "expect":
		if self.IsErr() {
			actual := ""
			...
			_msg := vm.eval(scope, call.Args[0]).AsString()
			panic(_msg + ": " + actual)
		}
	...
	case "or":
		...
		return vm.eval(scope, call.Args[0])
	case "is_ok":
