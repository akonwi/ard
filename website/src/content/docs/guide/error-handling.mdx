...
`T!Error` preserves the underlying Go error value through imported calls and generated Go APIs. Go `error` returns become `Void!Error`, and `(T, error)` returns become `T!Error`. Go parameters, fields, and callback arguments typed as `error` also use builtin `Error`.
...
## Nested Maybe Field Chains

A field chain through an Ard struct wrapped in `Maybe` can be used as a `try`
...
fn profile_age(user: UserWithProfile?) Int {
...
Accessing a required field such as `age: Int` produces `Int?`. An already-nullable
