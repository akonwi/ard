...
A captured variadic Go callable uses `...T` for its final element type, such as `fn(Str, ...Str) Str`. This type-only syntax does not declare an Ard variadic function. Calls may forward a list or list reference as one final spread argument; see [Go interop](../advanced/go-interop/#variadic-calls).
