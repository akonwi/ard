...
- [x] Port existing direct JS build/source-shape tests to the AIR backend or replace them with AIR parity/runtime tests.
  ...
  - [x] Obsolete checker-JS source-shape assertions were removed after AIR source/runtime parity coverage replaced them.
...
- [x] Add parity tests for maybe/result/try/match.
...
- [x] Add parity tests for JS externs.
  ...
  - [x] Added AIR Node runtime parity coverage for project extern Maybe/Result/struct/map return adapters.
...
- Public `javascript.Build` and `javascript.Run` now load/lower AIR and dispatch to `BuildProgram`/`RunProgram`.
- Removed the old checker-node JavaScript emitter and obsolete legacy source-shape tests.
