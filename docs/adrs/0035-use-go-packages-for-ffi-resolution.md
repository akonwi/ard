...
Generated output imports `encoding/json/v2` for union marshalling. Ard therefore requires Go 1.27 or newer, where JSONv2 is enabled in the baseline toolchain configuration and the package is available without an opt-in tag. The compiler does not inject `goexperiment.jsonv2`; only `[go] build_tags` entries are passed to `go/packages` resolution and generated builds. Explicitly disabling the Go baseline with `GOEXPERIMENT=nojsonv2` is incompatible with generated union code because that setting removes the `encoding/json/v2` package.
