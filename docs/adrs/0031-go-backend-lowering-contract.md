...
Marshalling and unmarshalling apply to `Dynamic`, structs, lists, maps, primitives, `Maybe`, and enums. `Dynamic` is the general-purpose marshalling vehicle. `Result` and `Union` are not marshalled directly; the checker rejects marshalling a `Result` or `Union`, or a type that contains one. A program that needs to serialize such a value converts it explicitly — for example into a struct or a `Dynamic` — so the output shape is chosen by the author rather than imposed by an internal representation.

The standard library conforms to this: `ard/json`'s existing `Result`-encoding paths and tests (`test_encode_result_ok`, `test_encode_result_err`) and union-based encoding through `Dynamic::from` are removed as part of this migration. Such values are converted explicitly before serialization rather than marshalled directly.
...
`runtime.Maybe[T]` implements JSON marshalling and unmarshalling: `none` marshals to `null`, `some(x)` marshals to `x` unwrapped, a present value unmarshals to `some`, and a JSON `null` or a missing field unmarshals to `none`. `Maybe` fields are not `omitempty`, so `none` is emitted as `null`. This is the one sanctioned exception to the reserved-method rule, because `Maybe` is a runtime type the backend owns.
