...
`ard.Maybe[T]` implements JSON marshalling and unmarshalling: `none` marshals to `null`, `some(x)` marshals to `x` unwrapped, a present value unmarshals to `some`, and a JSON `null` or a missing field unmarshals to `none`. `Maybe` fields are not `omitempty`, so `none` is emitted as `null`.
