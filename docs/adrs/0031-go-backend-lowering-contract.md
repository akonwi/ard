...
Marshalling and unmarshalling apply to `Dynamic`, structs, lists, maps, primitives, `Maybe`, `Result`, and enums. `Dynamic` is the general-purpose marshalling vehicle. `Union` is the one shape that is not marshalled directly; the checker rejects marshalling a `Union`, or a type that contains one, because a tagged-struct union has no single natural JSON shape. A program that needs to serialize a union converts it explicitly — for example into a struct or a `Dynamic` — so the output shape is chosen by the author rather than imposed by the internal representation.

The standard library conforms to this: union-based encoding through `Dynamic::from` is converted explicitly before serialization rather than marshalled directly.
