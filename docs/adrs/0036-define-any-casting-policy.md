...
Add a compiler-backed `ard/unsafe::cast` operation with this source-level shape:
...
`unsafe::cast` is the only built-in way to recover a typed value from `Any`. It is checked and fallible: success returns `some(value)`, failure returns `none`.
...
```ard
unsafe::cast<Str>(value) // Str?
unsafe::cast<User>(value) // User?
```

For a target type `T`, `unsafe::cast<T>(value)` succeeds when the boxed Go value is either:
...
```ard
unsafe::cast<mut Str>(value) // (mut Str)?
unsafe::cast<mut User>(value) // (mut User)?
```

For a target type `mut T`, `unsafe::cast<mut T>(value)` succeeds only when the boxed Go value is a non-nil pointer/reference representation of `T`.
...
- `unsafe::cast<T>(value)` where `value` contains a nil `*T` returns `none`.
- `unsafe::cast<mut T>(value)` where `value` contains a nil `*T` returns `none`.
...
`unsafe::cast` is a checked cast, not a decoder and not a coercion mechanism.

It does not convert between different scalar types:

```ard
let value: Any = 1
unsafe::cast<Float64>(value) // none
```
...
unsafe::cast<User>(value) // none, unless value actually contains a User
```

It does not implicitly convert named Go scalar types to their underlying Ard primitive. A boxed Go `type UserID string` is not a `Str` for `unsafe::cast<Str>` unless a later ADR explicitly adds named-scalar unwrapping. Use a Go shim or cast to the foreign named type when that type is nameable.

### Foreign Go types

`unsafe::cast` may target representable foreign Go types imported through `use go:`:

```ard
use go:time
use ard/unsafe

let duration = unsafe::cast<time::Duration>(value)
```
...
- The compiler must support `mut T` as an explicit type argument for `unsafe::cast`.
- The compiler must special-lower `unsafe::cast` because the target type argument determines the generated Go assertion code.
