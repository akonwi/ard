...
- Discarded after resuming: direct Result literals for `JsonToDynamic`, encoding/json v1, slow-path helper splits in direct decoders, scalar `DecodeIntErrors` raw-int fast path, and retrying raw map precheck in the direct string-int decoder all regressed.
