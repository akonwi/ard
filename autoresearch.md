...
- Correctness fix kept despite metric cost: typed lazy JSON int caches now reject invalid leading-zero numbers inside arrays/maps, with tests for `{"units":[01]}` and `{"counts":{"a":01}}`. Splitting zero/nonzero integer parsing and inlining the positive delimiter switch recovered part of the cost (~11.01 ms). Also kept sign-only negative rejection in typed caches despite metric cost. Noinline slow parser, cached first byte, post-scan leading-zero validation, delimiter switch reordering, bulk extern registration, disabled GC, direct Print emission, inlined typed-array positive parsing, field-extraction type switch, fixed-array cached int arrays, split negative slow helper, split negative digit checks, six-slot typed map duplicate tracker, and jsonObjectDynamic field reordering regressed.
