...
- Correctness fix kept despite metric cost: typed lazy JSON int caches now reject invalid leading-zero numbers inside arrays/maps, with tests for `{"units":[01]}` and `{"counts":{"a":01}}`. Splitting zero/nonzero integer parsing and inlining the positive delimiter switch recovered part of the cost (~11.01 ms). Noinline slow parser, cached first byte, post-scan leading-zero validation, delimiter switch reordering, bulk extern registration, disabled GC, and direct Print emission regressed.
