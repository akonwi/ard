...
- Discarded: filling lazy int-list output by index into a len-12 slice regressed; keep cap-12 append shape. Storing cached lazy object values as plain strings also regressed; keep `jsonDynamic` values. Removing the typed `[]int` direct decoder fast path after lazy JSON also regressed.
- Kept: top-level lazy object cache is now built during duplicate-name validation, avoiding a separate object-cache scan and improving to ~17.2 ms; the old separate parser was removed. Four-slot integrated cache and smaller top-level duplicate trackers regressed.
- Kept: the duplicate-name scanner now performs strict JSON validation itself, eliminating the extra `encoding/json.Valid` scan while preserving duplicate rejection, improving to ~14.12 ms. Added invalid-syntax tests. Nested object duplicate tracker tuned to 8 slots (~14.11 ms); 4/12 regressed.
- Kept: JSON key scanning now validates and extracts content in one pass, improving to ~14.02 ms. Removing the old unused helper and direct-close validator branches regressed.
- Kept: strict top-level validation now parses and caches typed int arrays and string-int maps in the same pass, improving to ~11.42 ms. Eager typed caching by rescanning had regressed; integrated parsing is the winning shape. Branch order was updated to check typed `[]int` before raw lazy JSON (~11.39 ms).
- Kept: cached lazy object values are stored in a single `[3]any` array instead of parallel raw/cached arrays, improving to ~11.19 ms and shrinking binary. Direct Result literals are used for cached field hits.
- Kept: typed validation scanners return directly on closing delimiters, improving to ~10.94 ms. The typed string-int map cache now falls back after eight keys instead of carrying map-fallback duplicate tracking, improving to ~10.84 ms. Index-filling/cap-limiting int arrays and four-slot duplicate tracking in the typed map scanner regressed.
- Discarded: returning named stdlib decoder functions from `int_list`/`string_int_map` passed the benchmark but failed VM checks; do not retry without cross-backend semantics work. Rechecking GC 100/1000, removing `RegisterExtern` locking, four-slot interface object cache, value-stored lazy objects, and using the output map for duplicate detection all regressed.
- Discarded after typed caching: direct Go helper lowerings for `decode.run(...field...).expect`, dead lazy-helper removal, removing top-level duplicate map fallback, positive-int parser split, branchless cached-field assignment, direct JsonToDynamic Result literals, duplicate-check unrolling, capacity-hint removal, constant cache bound, cached-object extraction unrolling, Ok constructors for typed hits, generic `MapKeys` map[string] specialization, and local-index typed array scanning all regressed.
- Correctness fix kept despite metric cost: typed lazy JSON int caches now reject invalid leading-zero numbers inside arrays/maps, with tests for `{"units":[01]}` and `{"counts":{"a":01}}`. Splitting zero/nonzero integer parsing and inlining the positive delimiter switch recovered part of the cost (~11.01 ms). Also kept sign-only negative rejection in typed caches; idx+1 lookahead recovered most of that cost (~11.07 ms). Noinline slow parser, cached first byte, post-scan leading-zero validation, delimiter switch reordering, bulk extern registration, disabled GC, direct Print emission, inlined typed-array positive parsing, no-space typed scanners, field-extraction type switch, fixed-array cached int arrays, split negative slow helper, split negative digit checks, six-slot typed map duplicate tracker, jsonObjectDynamic field reordering, from_json expect helper lowering, range-over-keys field extraction, ok-first Result expect, zero-branch reorder, and GOGC 200 recheck regressed.
- Broader runtime sanity after strict number fixes still looked healthy: decode_pipeline Go ~11.4 ms, sales ~4.7 ms, shape ~5.0 ms, word-frequency ~7.5 ms (3-run hyperfine; noisy but outputs verified).
