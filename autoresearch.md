...
- Discarded after current best: returning named decoder functions from stdlib helpers (compile failure), retrying `decode.run` inlining (compile failure), direct field FFI helpers around specialized decoders, small string sorting networks, index-only int-list loops, append-style int-list output, removing map capacity hints, `Ok`/`Err` final constructors, and nil error-slice checks all regressed.
